...
| 영역 | 설명 |
| --- | --- |
| 홈 `/` | 일반 사용자용 일정판. 사무실일정, 제작일정, 근무표, 휴가 탭을 제공합니다. |
| 디스플레이 `/display` | 삼성 Flip Pro/Tizen 브라우저를 고려한 상시 모니터 모드입니다. 일반 화면과 같은 기간 선택 UI에 자동 갱신을 더합니다. |
| 업로드 `/submit` | 이미지 업로드 또는 폼 입력으로 일정을 등록합니다. 디스플레이 모드에서 이동하면 업로드 후 `/display`로 돌아옵니다. |
| 사무실/제작 탭 | 탭의 `▼` 버튼에서 이번 주, 2주간, 이번 달, 다음 달 이후 일정을 선택해 보여줍니다. |
| 기간별 일정 | 이번 주·2주간 일정은 주간 카드, 이번 달·한 달 이후 일정은 월간 달력으로 표시합니다. |
| 편집/삭제 | 사무실/제작 일정에서 편집 모드를 켜 일정 내용을 수정하거나 삭제할 수 있습니다. |
...
- 일반 화면과 동일하게 `사무실일정 ▼` 또는 `제작일정 ▼`에서 기간을 선택합니다.
- 기간을 선택하면 드롭다운이 닫히고 선택한 일정 화면만 표시됩니다.
- 이번 달 일정은 해당 월 1일부터 말일까지, 한 달 이후 일정은 다음 달 1일 이후를 표시합니다.
- 삼성/Tizen 브라우저에서는 상세 팝업이 하나만 열리도록 처리하고, hover 대신 터치/클릭 중심으로 동작합니다.

## 일정 화면 사용법

1. 상단에서 `사무실일정 ▼` 또는 `제작일정 ▼`를 누릅니다.
2. 드롭다운에서 원하는 기간을 선택합니다.
   - `이번주일정`: 이번 주 월요일부터 일요일
   - `2주간 일정`: 이번 주와 다음 주
   - `이번달 일정`: 이번 달 1일부터 말일
   - `한달 이후 일정`: 다음 달 1일 이후의 모든 일정
3. 선택한 기간이 표시되면 드롭다운은 자동으로 닫힙니다.

월간 달력은 이번 주 카드와 같은 파란 계열을 사용합니다. 기간 이름과 중복되는 화면 제목, 월간 달력 바깥쪽의 중복 컨테이너는 표시하지 않습니다.
...
- 기간 드롭다운과 화면 전환은 클라이언트 상태만 사용하므로 추가 API 요청을 만들지 않습니다.
